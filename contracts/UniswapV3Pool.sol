...
import "./interfaces/IIbswapNftEquip.sol";
...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    //Contract address of Handling fee discount
    address public nftEquip;
        ...
        returns (int256 amount0, int256 amount1)
    {
        ...
        uint24 feeRate = fee;
        if (nftEquip != address(0)) {
            uint8 dr = IIbswapNftEquip(nftEquip).discountRate(recipient);
            if (dr >= 100) dr = 0;
            feeRate = (feeRate * (100 - dr)) / 100; // feeRate * (100 - dr) must < max(uint24)
        }

        (SwapCache memory cache, SwapState memory state) = _swapWhile(
            zeroForOne,
            amountSpecified,
            sqrtPriceLimitX96,
            exactInput,
    ...
    function _swapWhile(
    ...
    ) internal returns (SwapCache memory, SwapState memory) {
        ...
        while (
            ...
            (
                ...
                feeRate //containing discount rate
            );
